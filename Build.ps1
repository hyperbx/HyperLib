...
$buildPaths = @("HyperLib.CLI\bin\Publish\")
$patchVersion = ".github\workflows\Patch-Version.ps1"

if ($Help)
{
    echo "HyperLib Build Script"
    echo ""
    echo "Usage:"
    echo "-Archive - archives the build artifacts."
    echo "-BuildAll - build HyperLib with all available profiles."
    echo "-CommitID [id] - set the commit ID to use from GitHub for the version number."
    echo "-Configuration [name] - build HyperLib with a specific configuration."
    echo "-Clean - cleans the solution before building HyperLib."
    echo "-Help - display help."
    echo "-Profile [name] - build HyperLib with a specific profile."
    echo "-UseFullCommitID - use the full 40 character commit ID for the version number."
    echo "-Version [major].[minor].[revision] - set the version number for this build of HyperLib."
    exit
}
...
{
    echo ".NET SDK is required to build HyperLib."
